...
// 		header("Access-Control-Allow-Origin: https://pramesh.justcodenow.com/");
		...
		if($error == false)
		{
			...
			{	
				if($result->eStatus == 'Active')
				{
					...
					$data['iAdminId']	    = $result->vPassword;
				}
				else 
				{
					$data['estatus']     	= "1";
	...
	public function add_image_data()
	{	
		
		$iCompletestackId  = $_POST['iCompletestackId'];

	    $data['vTitle']              	= $_POST['vTitle'];
	    $data['vDesc']              	= $_POST['vDesc'];
		...
		if($_FILES['vImage']['name'] != "")
        {
            ...
            $directory = $base_path."/compeletestack/backend/image/all_image/";
		
            $save_path = base_url("/image/all_image/");
		...
		if($_FILES['vImage1']['name'] != "")
        {
            if($_FILES['vImage1']['type'] == 'image/jpg'){
                ...
                $ext = ".png";
            }else if($_FILES['vImage1']['type'] == 'image/webp'){
                $ext = ".webp";
            }

            $image = "image1_".time().$ext;
		...
		if($_FILES['vImage2']['name'] != "")
        {
            if($_FILES['vImage2']['type'] == 'image/jpg'){
                ...
                $ext = ".png";
            }else if($_FILES['vImage2']['type'] == 'image/webp'){
                $ext = ".webp";
            }

            $image = "image2_".time().$ext;
		...
		if($_FILES['vImage3']['name'] != "")
        {
            if($_FILES['vImage3']['type'] == 'image/jpg'){
                ...
                $ext = ".png";
            }else if($_FILES['vImage3']['type'] == 'image/webp'){
                $ext = ".webp";
            }

            $image = "image3_".time().$ext;
        ...
        {
            if($_FILES['vImage4']['type'] == 'image/jpg'){
                $ext = ".jpg";
            } else if($_FILES['vImage4']['type'] == 'image/jpeg'){
                $ext = ".jpeg";
            } else if($_FILES['vImage4']['type'] == 'image/png'){
                $ext = ".png";
            }else if($_FILES['vImage4']['type'] == 'image/webp'){
                $ext = ".webp";
            }

            $image = "image4_".time().$ext;
			$tmp_name = $_FILES["vImage4"]["tmp_name"];
            $base_path = $this->config->item('base_path');
            ...
            $data['vImage4'] =  $save_path.'/'.$image;
        }
		if($_FILES['vImage5']['name'] != "")
        {
            if($_FILES['vImage5']['type'] == 'image/jpg'){
                $ext = ".jpg";
            } else if($_FILES['vImage5']['type'] == 'image/jpeg'){
                $ext = ".jpeg";
            } else if($_FILES['vImage5']['type'] == 'image/png'){
                $ext = ".png";
            }else if($_FILES['vImage5']['type'] == 'image/webp'){
                $ext = ".webp";
            }

            $image = "image5_".time().$ext;
			$tmp_name = $_FILES["vImage5"]["tmp_name"];
            $base_path = $this->config->item('base_path');
		...
		if($iCompletestackId=="")
		{
			...
			else
			{
				$data = array();
				$data['Status'] 		= '1';
				$data['message']  		= 'Image Content data Not Added Please Try!';
			}
		}
		else
		{
			$where = array('iCompletestackId'=>$iCompletestackId);
	...
	public function all_image_get()
	{
		$iCompletestackId = $_GET['iCompletestackId'];

		if($_SERVER['REQUEST_METHOD']=='GET' && $iCompletestackId!="")
		{
			$result  = $this->content_model->get_by_id($iCompletestackId);
		...
		{
			$result     		= $this->content_model->get_by_all_image();
			$SingleData     	= $this->content_model->get_by_single_image();
			if(count($result) > 0)
			{
				$data['Status']     = '1';
	...
	{
		$iCompletestackId  = $this->input->post('iCompletestackId');
	
		$id = $this->content_model->delete_by_id($iCompletestackId);
	...
	public function stories_added()
	{
		...
		// if($_FILES['stories_video']['name'] != "")
        // {
		// 	$tmp_name = $_FILES["stories_video"]["tmp_name"];
        //     if($_FILES['stories_video']['type'] == 'video/mp4')
		// 	{
        //         $ext = ".mp4";
        //     } 
		// 	else if($_FILES['stories_video']['type'] == 'video/M4P')
		// 	{
        //         $ext = ".M4P";
        //     } 
		// 	else if($_FILES['stories_video']['type'] == 'image/M4P'){
        //         $ext = ".M4P";
        //     }
        ...
        //     $data['vStories_Video'] =  $save_path.'/'.$image;
        // }
		// ************************* Third Proccess END *************
		// ************************* Forth Proccess *************
	...
	public function wishlishadded()
	{
		...
		if(count($wishlistresult)>0)
		{
			$data = array();
			$data['Status'] 		= '0';
			$data['message']		= 'Single product get Successfully';
